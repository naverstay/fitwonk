...
$(window).load(function(){
    ...
    }).delegate('.select-programm .li-line .info .title', 'click', function () {
        var sld = $(this).closest('.slick-initialized').toggleClass('description_opened'), maxH = 0;

        sld.find('.li-line .li-block').css('height', 'auto');
        
        sld.find('.li-line').each(function (ind) {
            maxH = Math.max(maxH, $(this).height() - 50);
        });
...
$(function(){
    ...
    // mob menu
    $('body').delegate('.mobMenu', 'click', function () {
        $('body').toggleClass('open_menu');
    }).delegate('.main-menu', 'click', function () {
        $('body').removeClass('open_menu');
        initPracticeSlider();
    });
    // drop list
    $('body').delegate('.drop-button', 'click', function () {
        $(this).parents('.drop-block:first').toggleClass('closed').find('.slick-initialized').slick('setPosition');
    });
    // filter food search
...
function initPracticeSlider () {
    $('.foto-practice').each(function (ind) {
        ...
        if (practice.hasClass('slick-initialized')) {
            practice.slick('setPosition');
        } else {
            practice.slick({
                dots: true,
                mobileFirst: true,
